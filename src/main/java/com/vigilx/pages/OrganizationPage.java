 ...
 * unchanged. {@link #uploadLogo(String)} is additive: uploads a new organization logo via the
 * "Edit organization logo" icon and its "Change logo" crop dialog, guaranteeing the dialog is
 * closed afterward rather than assuming it closes itself.
 */
public class OrganizationPage extends BasePage {
 ...
 public boolean isOrganizationPageLoaded() {
   ...
   public boolean isOrganizationNameDisplayed() {
     ...
     * (click the exact "Save"). Earlier code stopped after the first click and force-closed
     * whatever dialog remained via {@link SoakUiUtils#closeOpenDialogs}, which - since that second
     * dialog was the still-open one - actually pressed Escape on the real save step and likely
     * discarded it instead of committing it; every save here is now confirmed by the actual "Logo
     * updated successfully" toast text (never a hard-coded toast id) before the window is
     * guaranteed closed. A blank/missing path skips cleanly (logged, returns {@code false}) so the
     * rest of the Organisation checks still run. Never throws into the caller.
     */
    public boolean uploadLogo(String logoPath) {
        ...
        try {
            // Confirmed live (screenshot evidence): clicking "Edit organization logo" fires the
            // hidden file input's own click handler, which - in a real, headed browser - opens a
            // genuine native OS "Open" file-picker window that nothing then closes. Wrapping the
            // click in waitForFileChooser intercepts that native dialog before it ever renders and
            // sets the file programmatically instead, so no OS window appears at all (stronger than
            // closing one after the fact). Falls back to locating the input directly if no
            // filechooser event fires (e.g. a build where the input is set without ever dispatching
            // a native chooser, as seen in some earlier probes).
            ...
            try {
                ...
                // No native chooser fired - the click above may or may not have gone through if it
                // threw for an unrelated reason, so make sure it has.
    ...
    private boolean waitForSuccessToast(String context, String before) {
        ...
        System.out.println("[ORGANISATION]   " + context + ": no notification observed within 6s.");
        return false;
    }
